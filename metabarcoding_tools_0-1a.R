################################################
# Custom functions for metabarcoding data analysis and mapping of chemistry and metabarcoding data
# by Alexander Keller (LMU München)
# keller@bio.lmu.de#### Custom functions
################################################

## Function to add leading zeros within plot names for plots from 1 to 9
addLeadingZeros <- function(x) {
  letters <- gsub("\\d", "", x)
  numbers <- as.integer(gsub("\\D", "", x))
  formatted_numbers <- sprintf("%02d", numbers)
  result <- paste0(letters, formatted_numbers)
  return(result)
}
...
label_sample_by_host <- function(phyloseq, hostcol, projcol="", idcol="", samplecol = ""){
  hostlist <- sample_data(phyloseq)[[hostcol]]
  projlist <- sample_data(phyloseq)[[projcol]]
  sample_labellist <- its2.species.new@sam_data[["new_name"]]  # Corrected this line
  
  if(length(idcol) != length(hostlist)){
    uniqIDs <- sample(1:99999,length(hostlist) ,replace=FALSE)
    uniqIDs <- sprintf(paste(hostlist, projlist, sample_labellist, "%05d", sep="|"), uniqIDs)

  }else{
    uniqIDs <- sprintf(paste(hostlist, projlist, sample_labellist, "%05d", sep="|"), idcol)
  }
